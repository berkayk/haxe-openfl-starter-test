...
class ApplicationMain
{
	...
	public static function create(config):Void
	{
		...
		app.meta["packageName"] = "com.example.Bootstrap";
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("Bootstrap")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess Bootstrap.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"Bootstrap\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"Bootstrap\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends Bootstrap {}
#else
class DocumentClass
{
